...
float bilateralFilter(vec2 coord, vec2 texelSize)
{
	...
	int numYSamples = int(scale*100*pow(1.0f - depth,2.0f) + 40);
	//int numYSamples = int(scale*100*pow(1.0f - depth,2.0f) + 80);

	//float spatialScale = numYSamples*texelSize.x/3.0f;
	float spatialScale = 0.008;
	//float tonalScale = 0.025f/3.0f;

	for( int j = -numYSamples; j <= numYSamples; j++ )
	{
		vec2 sampleCoord = texCoord + vec2(0, j*texelSize.y);
